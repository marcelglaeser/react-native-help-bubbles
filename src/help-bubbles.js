...
animateMove(obj = {}) {
  ...
  if (stepNumLeft < 0) {
    ...
    if (stepNumLeft > Dimensions.get('window').width - 24) {
      stepNumLeft = Dimensions.get('window').width - 24;
    }
  }
  ...
  var relative2Bottom = Math.abs(centerPoint.y - Dimensions.get('window').height);
  var relative2Right = Math.abs(centerPoint.x - Dimensions.get('window').width);
  ...
  if(obj.center == 'vertical' || obj.center == 'horizontal_vertical' ) {
    whereVerticalPlace = 'center';
  }
  if(obj.center == 'horizontal' || obj.center == 'horizontal_vertical' ) {
    whereHorizontalPlace = 'center';
  }
  ...
  switch (whereVerticalPlace) {
    ...
    case 'top':
    tooltip.bottom = Dimensions.get('window').height - obj.top + margin;
  ...
  switch (whereHorizontalPlace) {
    ...
    case 'left':
    tooltip.right = Math.max(Dimensions.get('window').width - (obj.left + obj.width), 0);
    tooltip.right = tooltip.right === 0 ? tooltip.right + margin : tooltip.right;
    tooltip.maxWidth = Dimensions.get('window').width - tooltip.right;
    ...
    tooltip.maxWidth = Dimensions.get('window').width - tooltip.left - margin;
...
render() {
  return (
    <View style={[styles.container, this.container, {zIndex}]}>
